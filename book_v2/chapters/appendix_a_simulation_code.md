...
1. **Matched training budgets.** Both controllers receive the same number of fitness evaluations (`--train-evals`). The budget is fixed before either controller is evolved. We did not keep training until the quantum controller won.
...
These results do not support monotonic scaling with latent dimension. Increasing from 5 to 6 to 7 latent dimensions does not produce monotonically increasing advantage. The effect is non-monotonic, peaking at an intermediate latent gap of approximately 3 extra dimensions.
